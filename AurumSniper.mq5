...
//+------------------------------------------------------------------+
#property description "AurumSniper Institutional V15.30: SMC + Opwens Candlestick Triggers & Classical Confluence"
// CHANGELOG V15.30:
...
input int      InpMagicNumber             = 777999; // Magic Number de identificación
input double   InpLotSize                 = 0.01;   // Lote base para operativa estándar
...
input double   InpMaxDailyLossUSD         = 150.0;  // [V15] Límite Monetario Máximo de Pérdida Diaria ($ USD)
...
void DrawPositionLevels() {
   ...
   datetime pos_time_cdmx = pos_time + (TimeLocal() - TimeCurrent());
   ...
   string entry_tt = StringFormat("⏰ ENTRADA %s #%I64u @ %.5f [%s CDMX] | En Mercado: %s (%d velas %s)",
                                  (type == POSITION_TYPE_BUY ? "BUY" : "SELL"),
                                  active_ticket,
                                  entry,
                                  TimeToString(pos_time_cdmx, TIME_MINUTES),
                                  dur_txt,
                                  bars_held,
   ...
   DrawChartText("tp_lvl_txt_entry", bar0_time, entry, StringFormat("  ⏰ %s [%s CDMX] | %s (%d velas)", (type == POSITION_TYPE_BUY ? "BUY" : "SELL"), TimeToString(pos_time_cdmx, TIME_MINUTES), dur_txt, bars_held), clrWhite);
...
void UpdateDashboard() {
   ...
   if(act_ticket > 0) {
      ...
      string timing_lbl_txt = StringFormat("⏰ Entrada: %s CDMX | Mercado: %s (%d velas %s)",
                                           TimeToString(pos_time_cdmx, TIME_MINUTES),
                                           pos_dur_str,
                                           pos_bars_held,
                                           EnumToString(_Period));
